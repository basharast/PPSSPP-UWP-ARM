...
UI::EventReturn SavedataScreen::OnSearch(UI::EventParams &e) {
	...
	if (System_GetPropertyBool(SYSPROP_HAS_TEXT_INPUT_DIALOG)) {
		...
		auto filtersPopup = new UI::PopupTextInputChoice(&searchFilter_, di->T("Filter"), "", 64, screenManager());
		filtersPopup->OnChange.Add([=](UI::EventParams& e) {
			System_PostUIMessage("savedatascreen_search", StripSpaces(searchFilter_).c_str());
			return UI::EVENT_DONE;
		});
		filtersPopup->Click();
	}
