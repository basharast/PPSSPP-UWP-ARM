...
DWORD(WINAPI* GetLogicalDrivesPtr)();

#if PPSSPP_PLATFORM(WINDOWS)
// Returns a vector with the device names
std::vector<std::string> GetWindowsDrives()
{
	if (GetLogicalDrivesPtr == NULL) {
		GetLogicalDrivesPtr = GetLogicalDrives;
	}

	std::vector<std::string> drives;

#if PPSSPP_PLATFORM(UWP)
	if (!GetLogicalDrivesPtr) {
		return drives;
	}
	DWORD logicaldrives = GetLogicalDrivesPtr();
	for (int i = 0; i < 26; i++)
	{
