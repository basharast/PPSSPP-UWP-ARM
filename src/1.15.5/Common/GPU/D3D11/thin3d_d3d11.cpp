...
public:
	...
	std::string GetInfoString(InfoField info) const override {
		...
		case ID: return std::to_string(adapterId_);
		case REV: return std::to_string(adapterRev_);
		case SYSMEM: return std::to_string(adapterSysMem_);
		case VIDMEM: return std::to_string(adapterVidMem_);
		case SHAMEME: return std::to_string(adapterShaMem_);
		case VENDOR: return "";
		case DRIVER: return "-";
...
D3D11DrawContext::D3D11DrawContext(ID3D11Device *device, ID3D11DeviceContext *deviceContext, ID3D11Device1 *device1, ID3D11DeviceContext1 *deviceContext1, IDXGISwapChain *swapChain, D3D_FEATURE_LEVEL featureLevel, HWND hWnd, std::vector<std::string> deviceList, int maxInflightFrames)
	...
	if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr)) {
			...
			adapterId_ = (int)desc.VendorId;
			adapterRev_ = (int)desc.Revision;
			adapterSysMem_ = static_cast<long>(desc.DedicatedSystemMemory);
			adapterVidMem_ = static_cast<long>(desc.DedicatedVideoMemory);
			adapterShaMem_ = static_cast<long>(desc.SharedSystemMemory);

			if (adapterSysMem_ < 0) {
				adapterSysMem_ = 0;
			}
			else {
				adapterSysMem_ = (int)(adapterSysMem_ / 1024 / 1024);
			}
			if (adapterVidMem_ < 0) {
				adapterVidMem_ = 0;
			}
			else {
				adapterVidMem_ = (int)(adapterVidMem_ / 1024 / 1024);
			}
			if (adapterShaMem_ < 0) {
				adapterShaMem_ = 0;
			}
			else {
				adapterShaMem_ = (int)(adapterShaMem_ / 1024 / 1024);
			}
...
Framebuffer *D3D11DrawContext::CreateFramebuffer(const FramebufferDesc &desc) {
	...
	descColor.Width = desc.width;
	descColor.Height = desc.height;
	if (descColor.Width > GetMaxTextureDimension(featureLevel_)) {
		descColor.Width = (int)GetMaxTextureDimension(featureLevel_);
	}
	...
	if (desc.z_stencil) {
		...
		if (descDepth.Width > GetMaxTextureDimension(featureLevel_)) {
			descDepth.Width = (int)GetMaxTextureDimension(featureLevel_);
		}
		descDepth.MipLevels = 1;
		descDepth.ArraySize = 1;
