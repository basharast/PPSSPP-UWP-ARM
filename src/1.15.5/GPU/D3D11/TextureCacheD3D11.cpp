...
void TextureCacheD3D11::BuildTexture(TexCacheEntry* const entry) {
	...
	if (plan.depth == 1) {
		...
		//if (desc.Width >= 128) {
			//desc.Width = round(desc.Width * 0.8);
			//desc.Height = round(desc.Height * 0.8);
		//}
		//if (subresData->SysMemPitch >= 256) {
			//subresData->SysMemPitch = round(subresData->SysMemPitch * 0.8);
		//}
		auto hrr = device_->CreateTexture2D(&desc, subresData, &tex);
		if (FAILED(hrr)) {
			ERROR_LOG(G3D, "CreateTexture2D Failed error: %d.", hrr);
			ERROR_LOG(G3D, "subresData: Width (%d), Height(%d).", hrr);
			if ((hrr == DXGI_ERROR_DEVICE_REMOVED || hrr == DXGI_ERROR_DEVICE_RESET))
			{
				WARN_LOG(G3D, "Reason: DEVICE_REMOVED or DEVICE_RESET.");
			}
			if (g_Config.bAssertTextureCache) {
				WARN_LOG(G3D, "CreateTexture2D, Assertion on.");
				ASSERT_SUCCESS(hrr);
			}
			else {
				WARN_LOG(G3D, "CreateTexture2D, Assertion off.");
			}
		}
		texture = tex;
	}
		...
		desc.MipLevels = 1;
		desc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
		auto hdr = device_->CreateTexture3D(&desc, subresData, &tex);
		if (FAILED(hdr)) {
			ERROR_LOG(G3D, "CreateTexture3D Failed error: %d.", hdr);
			if ((hdr == DXGI_ERROR_DEVICE_REMOVED || hdr == DXGI_ERROR_DEVICE_RESET))
			{
				WARN_LOG(G3D, "Reason: DEVICE_REMOVED or DEVICE_RESET.");
			}
			if (g_Config.bAssertTextureCache) {
				WARN_LOG(G3D, "CreateTexture3D, Assertion on.");
				ASSERT_SUCCESS(hdr);
			}
			else {
				WARN_LOG(G3D, "CreateTexture3D, Assertion off.");
			}
		}
		texture = tex;
