...
rotateVBO:
		...
		if (!vb_) {
			...
			if (useElements) {
				...
				ID3D11Buffer* ibuf = pushInds_->Buf(); // Get the index buffer from pushInds_
				context_->IASetIndexBuffer(ibuf, DXGI_FORMAT_R16_UINT, iOffset); // Set the correct index buffer
