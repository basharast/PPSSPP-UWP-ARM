...
std::vector<PSPFileInfo> VirtualDiscFileSystem::GetDirListing(const std::string &path, bool *exists) {
...
#else
	hFind = FindFirstFileExW(w32path.c_str(), FindExInfoStandard, &findData, FindExSearchNameMatch, NULL, 0);
#endif
#else
	...
	for (BOOL retval = 1; retval; retval = FindNextFileW(hFind, &findData)) {
