...
void NativeInit(int argc, const char* argv[], const char* savegame_dir, const char* external_dir, const char* cache_dir) {
...
#endif
	if (g_Config.currentDirectory == Path("/")) {
		g_Config.currentDirectory = g_Config.memStickDirectory;
	}
...
#elif (defined(_M_ARM) && !defined(_DEBUG))
	...
	SetGPUBackend((GPUBackend) g_Config.iGPUBackend, g_Config.sD3D11Device);
	renderCounter = 0;
