...
void ConfirmMemstickMoveScreen::CreateViews() {
	...
	if (oldMemstickFolder.empty()) {
		oldMemstickFolder = Path(GetLocalFolder());
	}
...
void ConfirmMemstickMoveScreen::FinishFolderMove() {
	...
	g_Config.defaultCurrentDirectory = !g_Config.memStickDirectory.empty() ? g_Config.memStickDirectory : Path("/");

	InitSysDirectories(true);
