...
void TextEdit::FocusChanged(int focusFlags) {
	TextEdit::focusFlags_ = focusFlags;

	//if (focusFlags & FF_LOSTFOCUS)
	TextEdit::focusFlagsCalls_++;
	concurrency::create_task([&] {
		std::this_thread::sleep_for(std::chrono::milliseconds(1)); 
	if (TextEdit::focusFlagsCalls_ == 1) {
		if (this->GetCurrentFocus() == FF_GOTFOCUS) {
#if defined(_M_ARM) || defined(BUILD14393)
			globalTextEdit = this;
			System_NotifyUIState("skeyboard");
#else
			globalTextEdit = this;
			System_NotifyUIState("skeyboard");
#endif
			...
			System_NotifyUIState("hkeyboard");
		}
	}
	TextEdit::focusFlagsCalls_--;
	return 0; 
...
int TextEdit::GetCurrentFocus() {
	return TextEdit::focusFlags_;
}
