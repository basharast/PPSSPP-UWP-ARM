...
public:
	SliderFloatPopupScreen(float *value, float minValue, float maxValue, float defaultValue, const std::string &title, float step = 1.0f, const std::string &units = "", bool liveUpdate = false, bool updateUI = false)
		: PopupScreen(title, "OK", "Cancel"), units_(units), value_(value), originalValue_(*value), minValue_(minValue), maxValue_(maxValue), defaultValue_(defaultValue), step_(step), liveUpdate_(liveUpdate), updateUI_(updateUI) {}
	void CreatePopupContents(UI::ViewGroup *parent) override;
...
public:
	...
	void SetLiveUpdate(bool update, bool updateUI = false) {
		liveUpdate_ = update;
		updateUI_ = updateUI;
	}
