...
void LogoScreen::DrawForeground(UIContext &dc) {
	...
	if (createdBy[0] == '\0') {
		snprintf(createdBy, sizeof(createdBy), "%s Henrik Rydg%c%crd", "Created by", 0xC3, 0xA5);
	}
...
void CreditsScreen::CreateViews() {
	...
	int rightYOffset = 0;
	//if (!System_GetPropertyBool(SYSPROP_APP_GOLD)) {
		root_->Add(new Button(cr->T("Support Project"), new AnchorLayoutParams(260, 64, NONE, NONE, 10, 84, false)))->OnClick.Handle(this, &CreditsScreen::OnSupport);
		rightYOffset = 74;
	//}
...
#if defined(_M_ARM)
		"",
		cr->T("portedtoARM", "ARM32 (UWP) release:"),
		"Bashar Astifan",
		"",
		cr->T("contributors", "Contributors:"),
#endif
