...
UI::EventReturn UIScreen::OnOK2(UI::EventParams& e) {
	TriggerFinish(DR_OK);
	recreateView = true;
	return UI::EVENT_DONE;
}
...
void PopupScreen::update() {
	...
	++frames_;
	if (finishFrame_ >= 0) {
		float leadOut = bezierEaseInOut((frames_ - finishFrame_) * (1.0f / 0.01f));
		animatePos = 1.0f - leadOut;

		if (frames_ >= finishFrame_ + 0.01f) {
			// Actual finish happens here.
			screenManager()->finishDialog(this, finishResult_);
		}
	} else if (frames_ < 0.01f) {
		float leadIn = bezierEaseInOut(frames_ * (1.0f / 0.01f));
		animatePos = leadIn;
	}
